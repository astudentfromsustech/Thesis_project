...
if __name__ == '__main__':
    ...
    dest_dir = root.parent / 'results' / '11_call_interaction_domains_STEP1_mergeIntervals'
    ...
    for num in num_list:
        ...
        for chro in chro_list:
            ...
            merged_intervals_Chro = merge_intervals(intervals_Chro)
            merged_intervals_Chro.insert(0, 'chromosome', chro)
            # print(merged_intervals_Chro.shape)
            # print(merged_intervals_Chro.head())
            intervals_merged = pd.concat([intervals_merged, merged_intervals_Chro])
        # print(intervals_merged.shape)
        # print(intervals_merged)
        intervals_merged.to_csv(dest_dir / f'CDS0{num}D_PETcount_thresh{PETcount_thresh}_loop_span_thresh{loop_span_thresh}_mergedIntervals.bed', header=None, index=None, sep='\t')
