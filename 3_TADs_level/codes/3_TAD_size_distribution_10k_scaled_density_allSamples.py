...
if __name__ == '__main__':
    ...
    ac_WT_kde = gaussian_kde(ac_WT_TAD_addSize['size'])
    ac_WT_x_range = np.linspace(ac_WT_min, ac_WT_max, 1000)
    ac_WT_y_vals = ac_WT_kde(ac_WT_x_range)
    ac_WT_y_vals = ac_WT_y_vals / ac_WT_y_vals.max()
    ...
    ac_HS_kde = gaussian_kde(ac_HS_TAD_addSize['size'])
    ac_HS_x_range = np.linspace(ac_HS_min, ac_HS_max, 1000)
    ac_HS_y_vals = ac_HS_kde(ac_HS_x_range)
    ac_HS_y_vals = ac_HS_y_vals / ac_HS_y_vals.max()
    ...
    plt.plot(ac_WT_x_range, ac_WT_y_vals, lw=1, color=ac_WT_color)
    plt.plot(ac_HS_x_range, ac_HS_y_vals, lw=1, color=ac_HS_color)
    ...
    plt.show()
    # fig.savefig(dest_dir / f'{WT_num}_{HS_num}_{ac_WT_num}_{ac_HS_num}_scaled_density_TAD_size_allSamples.png', dpi=300, bbox_inches='tight')