...
from scipy.stats import pearsonr, spearmanr, gaussian_kde, ttest_rel, wilcoxon
...
if __name__ == '__main__':
    ...
    for chro in chro_list:
        ...
        WT_src_dir = root.parent / 'results' / f'ac_me3_downsample_WT_me3_ac_combined_10000_targetChroms_normalize0_1_correctionKR_min30k_max100k_step10k_thresh0.1_delta0.01_dfr'
        HS_src_dir = root.parent / 'results' / f'ac_me3_downsample_HS_me3_ac_combined_10000_targetChroms_normalize0_1_correctionKR_min30k_max100k_step10k_thresh0.1_delta0.01_dfr'
        dest_dir = root.parent / 'results' / f'3_draw_scatterPlot_withColorDensity_allChorms_of_IS_for_ac_me3_withoutLabels'
        dest_dir.mkdir(parents=True, exist_ok=True)

        WT_IS = pd.read_csv(WT_src_dir / f'{WT_num}_me3_ac_score.bedgraph', header=None, sep='\t')
        ...
        HS_IS = pd.read_csv(HS_src_dir / f'{HS_num}_me3_ac_score.bedgraph', header=None, sep='\t')
    ...
    print(combined_result.shape)
    # print(combined_result)
    t_statistic, p_value = ttest_rel(combined_result['score_y'], combined_result['score_x'])
    print(t_statistic, p_value)
    w_statistic, w_p_value = wilcoxon(combined_result['score_y'] - combined_result['score_x'])
    print(w_statistic, w_p_value)
    ...
    ax.set_yticklabels('')
    cbar.set_ticks([0.4, 0.8, 1.2])
    cbar.set_ticklabels('')
    cbar.set_label('')
    plt.show()
    # plt.savefig(dest_dir / f'ScatterPlot_with_correlation_of_IS_vector_CDS0{WT_num}D_CDS0{HS_num}D_AllChroms_pearson_{round(pearson_corr, 2)}_spearman_{round(spearman_corr, 2)}_withoutLabel.png', dpi=300,
    #             bbox_inches='tight')
