...
if __name__ == '__main__':
    ...
    for merged_loop_countThresh in merged_loop_countThresh_list:
        ...
        WT_inter_A = WT_loops[WT_loops['type'] == 'inter_A']
        # print(WT_inter_A.shape)
        # print(WT_inter_A.head())
        # print(WT_inter_A['type'].value_counts())
        WT_inter_A_chrom = WT_inter_A[(WT_inter_A['chromosome1'] == chrom) & (WT_inter_A['chromosome2'] == chrom)]
        # print(WT_inter_A_chrom.shape)
        # print(WT_inter_A_chrom.head(10))
        # print(WT_inter_A_chrom['type'].value_counts())


        min_id_number = int(intersected_A_chrom['ID'].str.extract('(\d+)').astype(int).min())
        print(min_id_number)
        WT_inter_A_chrom_matrix = pd.DataFrame(0, index=range(intersected_A_chrom.shape[0]), columns=range(intersected_A_chrom.shape[0]))
        # print(WT_inter_A_chrom_matrix.shape)
        # print(WT_inter_A_chrom_matrix)

        WT_inter_A_chrom_matrix_filled = fill_rpm_dataframe(WT_inter_A_chrom, WT_inter_A_chrom_matrix, min_id_number)
        print(WT_inter_A_chrom_matrix_filled.shape)
        print(WT_inter_A_chrom_matrix_filled)
        ...
        HS_loops = pd.read_csv(
            ...
            sep='\t')
        HS_inter_A = HS_loops[HS_loops['type'] == 'inter_A']
        # print(HS_inter_A.shape)
        # print(HS_inter_A.head())
        # print(HS_inter_A['type'].value_counts())
        HS_inter_A_chrom = HS_inter_A[(HS_inter_A['chromosome1'] == chrom) & (HS_inter_A['chromosome2'] == chrom)]
        # print(HS_inter_A_chrom.shape)
        # print(HS_inter_A_chrom.head(10))
        # print(HS_inter_A_chrom['type'].value_counts())

        min_id_number = int(intersected_A_chrom['ID'].str.extract('(\d+)').astype(int).min())
        print(min_id_number)
        HS_inter_A_chrom_matrix = pd.DataFrame(0, index=range(intersected_A_chrom.shape[0]),
                                           columns=range(intersected_A_chrom.shape[0]))
        # print(HS_inter_A_chrom_matrix.shape)
        # print(HS_inter_A_chrom_matrix)

        HS_inter_A_chrom_matrix_filled = fill_rpm_dataframe(HS_inter_A_chrom, HS_inter_A_chrom_matrix, min_id_number)
        print(HS_inter_A_chrom_matrix_filled.shape)
        print(HS_inter_A_chrom_matrix_filled)

        minus = np.log2((HS_inter_A_chrom_matrix_filled + 1) / (WT_inter_A_chrom_matrix_filled + 1)) * 2
        print(minus.shape)
        print(minus)

        plt.figure(figsize=(8, 8), dpi=200)  # Set the figure size as desired
        # sns.heatmap(minus, annot=False, cmap='coolwarm', cbar=True)
        ...
        plt.title(f'log2[(HS_inter_A + 1) / (WT_inter_A + 1)]\n(using all inter_A loops with PET >= {merged_loop_countThresh})')  # Adjust the title position if necessary
