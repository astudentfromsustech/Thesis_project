...
if __name__ == '__main__':
    ...
    dest_dir = root.parent / 'results' / '8_domain_cluster_WT_HS_statistics2_number_of_interaction_barPlot'
    ...
    for num_pair in num_list:
        ...
        # WT_inter_A_cluster_numEdge = WT_inter_A_cluster_df['num_anchors'].value_counts().sort_index()
        WT_inter_A_cluster_numEdge = WT_inter_A_cluster_df['num_interaction'].value_counts().sort_index()
        # HS_inter_A_cluster_numEdge = HS_inter_A_cluster_df['num_anchors'].value_counts().sort_index()
        ...
        combined_numEdge.columns = ['WT_num', 'HS_num']
        print(combined_numEdge)
        print(combined_numEdge.sum())
        total_numEdge = combined_numEdge.multiply(combined_numEdge.index, axis=0).sum()
        print(total_numEdge)
        # combined_numEdge = pd.concat([WT_inter_A_cluster_numEdge, HS_inter_A_cluster_numEdge], axis=1).fillna(0)
        # print(combined_numEdge)
        ...
        combined_numEdge.plot(kind='bar', ax=ax, color=[WT_color, HS_color])
        ax.set_title('')
        ax.set_xlabel('Interaction frequency in a cluster')
        ax.set_ylabel('Count')
        ax.legend(['WT_inter_A_cluster', 'HS_inter_A_cluster'], framealpha=0)
        plt.xticks(rotation=0)
        # plt.show()
        fig.savefig(dest_dir / f'CDS0{WT_num}D_CDS0{HS_num}D_inter_A_numEdge_statistics_barPlot_Domain_PETcount{Domain_PETcount_thresh_inter_active}_Domain_distance{Domain_distance_thresh_inter_active}.png')
    ...
    for num_pair in num_list:
        ...
        # plt.show()
        fig.savefig(
            dest_dir / f'CDS0{WT_num}D_CDS0{HS_num}D_inter_B_numEdge_statistics_barPlot_Domain_PETcount{Domain_PETcount_thresh_inter_inactive}_Domain_distance{Domain_distance_thresh_inter_inactive}.png')
