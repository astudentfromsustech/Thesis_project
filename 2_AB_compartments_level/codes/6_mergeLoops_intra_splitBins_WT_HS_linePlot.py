...
if __name__ == '__main__':
    ...
    dest_dir = root.parent / 'results' / '6_mergeLoops_intra_splitBins_WT_HS_linePlot'
    ...
    for num_pair in num_list:
        ...
        # print(intersected_B_addRPKM_WT['ID'].value_counts().sort_index())
        ...
        # print(intersected_B_addRPKM_HS['ID'].value_counts().sort_index())
        #
        #
        ...
        plt.plot(intersected_B_addRPKM_WT_average_smoothed, color='black', linewidth=1, label='WT_intra_B')
        plt.plot(intersected_B_addRPKM_HS_average_smoothed, color='black', linestyle='--', linewidth=1, label='HS_intra_B')
        ax.set_ylabel('RPKM (loop coverage)', fontsize=12)
        ax.set_xlabel('Bin', fontsize=12)
        plt.legend(loc='upper right', frameon=False)
        # plt.show()

        plt.savefig(dest_dir / f'CDS0{num_WT}D_CDS0{num_HS}D_intersected_B_binNum_{num_bins}_extraNum_{extra_bins}_merged_loop_countThresh{merged_loop_countThresh}.png', dpi=300, bbox_inches='tight')
    ...
    for num_pair in num_list:
        ...
        intra_A_accumulated_WT = pd.read_csv(
            src_dir / f'CDS0{num_WT}D_intra_A_PETcount_thresh{PETcount_thresh}_merged_loop_countThresh{merged_loop_countThresh}_accumulatedCount.bedgraph',
            sep='\t')
        ...
        for chro in chro_list:
            ...
            intersected_A_Chro_split_AccumulatedCoverage_addRPKM[
                ...
                lambda row: row['accumulated_coverage'] / (row['bin_size'] / 1000) / (unique_PET_dir[num_WT] / 1000000),
                axis=1)
        ...
        intersected_A_allChroms_split_AccumulatedCoverage_addRPKM.to_csv(
            dest_dir / f'CDS0{num_WT}D_intersected_A_allChroms_split_AccumulatedCoverage_addRPKM_binNum_{num_bins}_extraNum_{extra_bins}_merged_loop_countThresh{merged_loop_countThresh}.txt',
            index=None, sep='\t')
        intersected_A_addRPKM_WT = pd.read_csv(
            dest_dir / f'CDS0{num_WT}D_intersected_A_allChroms_split_AccumulatedCoverage_addRPKM_binNum_{num_bins}_extraNum_{extra_bins}_merged_loop_countThresh{merged_loop_countThresh}.txt',
            sep='\t')
        print(intersected_A_addRPKM_WT.shape)
        print(intersected_A_addRPKM_WT['ID'].value_counts().sort_index())
        ...
        intersected_A_addRPKM_HS = pd.read_csv(
            dest_dir / f'CDS0{num_HS}D_intersected_A_allChroms_split_AccumulatedCoverage_addRPKM_binNum_{num_bins}_extraNum_{extra_bins}_merged_loop_countThresh{merged_loop_countThresh}.txt',
            sep='\t')
        print(intersected_A_addRPKM_HS.shape)
        # print(intersected_A_addRPKM_HS['ID'].value_counts().sort_index())
        ...
        x_labels = ['', '0', '30', '60', '90', '120', '150', '180', '210', '240', '270', '300', '']  # Replace with your actual labels
        plt.xticks(ticks=x_positions, labels=x_labels)
        plt.plot(intersected_A_addRPKM_WT_average_smoothed, color='orange', linewidth=1, label='WT_intra_A')
        plt.plot(intersected_A_addRPKM_HS_average_smoothed, color='orange', linestyle='--', linewidth=1, label='HS_intra_A')
        ax.set_ylabel('RPKM (loop coverage)', fontsize=12)
        ax.set_xlabel('Bin', fontsize=12)
        plt.legend(loc='upper right', frameon=False)
        # plt.show()
        plt.savefig( dest_dir / f'CDS0{num_WT}D_CDS0{num_HS}D_intersected_A_binNum_{num_bins}_extraNum_{extra_bins}_merged_loop_countThresh{merged_loop_countThresh}.png',
